...
{
    unsigned* bucket = m_Buckets[index / m_BucketSize];
    if (!bucket)
        return false;
...
{
    size_t count = 0;
    for (vector<unsigned*>::const_iterator it = m_Buckets.begin(); it != m_Buckets.end(); it++)
    {
        if (*it)
            count++;
    }
...
{
    for (vector<unsigned*>::iterator it = m_Buckets.begin(); it != m_Buckets.end(); it++)
    {
        if (*it)
        {
            delete[] *it;
...
{
    unsigned* bucket = new unsigned[m_BucketSize / (8 * sizeof(unsigned))];
    m_Buckets[index] = bucket;
    fill(&bucket[0], &bucket[m_BucketSize / (8 * sizeof(unsigned))], 0);
    return bucket;
}
